...
function removeFirstOccurrences(str, value) {
  const replacement = str.indexOf(value);
  if (replacement === -1) {
    return str;
  }
  return str.slice(0, replacement) + str.slice(value.length + replacement);
}
...
function removeLastOccurrences(str, value) {
  const replacement = str.lastIndexOf(value);
  if (replacement === -1) {
    return str;
  }
  return str.slice(0, replacement) + str.slice(value.length + replacement);
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str.charAt(i) === str.charAt(i).toUpperCase()) {
      inversedStr += str[i].toLowerCase();
    } else {
      inversedStr += str[i].toUpperCase();
    }
  }
  return inversedStr;
}
...
function extractNameFromTemplate(value) {
  const finalValue = value.replace('Hello, ', '');
...
function extractEmails(str) {
  return str.split(';').map((w) => w.split(',').join(''));
}
